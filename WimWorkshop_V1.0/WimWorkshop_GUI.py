...
class ui_wim_index_maker:
    ...
    def show_wim_index(self):
        # 映像序号选择
        self.spinbox = ttk.Spinbox(self.page, width=4, from_=0, to=0)
        self.spinbox.place(x=self.place_x, y=self.place_y)
        self.spinbox_text = tk.Label(self.page, text="分卷号：")
        self.spinbox_text.place(x=self.place_x - 70, y=self.place_y)
        # 映像名称输入框
...
class ui_choose:
    def __init__(self, page, place_x, place_y, text, open_type):
        ...
        :param text: 文字提示内容
        :param open_type: 选择类型。若为"open_file"则为选择文件；若为"save_file"则为保存文件；若为"folder"则选择文件夹
        """
        self.page = page
        self.text = text
        self.place_x = place_x
        ...
        if self.open_type == "open_file":
            self.dialog = self.openfile_dialog
        elif self.open_type == "save_file":
            self.dialog = self.savefile_dialog
    ...
    def savefile_dialog(self):
        """
        调用此函数，创建一个保存wim文件的对话框。
        :return:
        """
        self.path = filedialog.asksaveasfilename(title="选择保存位置", filetypes=[("wim映像文件", "*.wim")], defaultextension=".wim")
        self.update_input(self.path)
...
class WimWorkshop:
    def __init__(self):
        ...
        # 初始化控件
        self.ui1 = ui_choose(self.tab1, 5, 10, "wim映像文件", "open_file")
        ...
        self.ui4 = ui_choose(self.tab2, 5, 40, "wim文件保存", "save_file")
        ...
        self.ui6 = ui_wim_index_maker(self.tab2, 500, 40, True)       # 标签页2
        ...
        self.load_wim_button_tab2 = ttk.Button(self.tab2, width=25, text="加载指定的映像", command=self.load_wim_button_tab2)
        self.load_wim_button_tab2.place(x=430, y=10)
    ...
    def load_wim_button_tab2(self):
        try:
            Checker.path_check(self.ui3.get_result())
        except Exception as err:
            error_dialog(err)
        else:
    ...
    def start_work(self, tab, work, wim_index):
        ...
        if work == "extract":
            ...
            wim_name = self.ui6.name_input_box.get()
            wim_description = self.ui6.Description_input_box.get()
            wim_maker.make_dism(self.wim_path, self.folder_path, wim_name, wim_description, False)
        else:
            return
